...
def fetch_media(handle,kind,mid):
    ...
    return {'media_id':v.get('id'),'desc':v.get('desc')[:3000] if v.get('desc') else None,'create_time':v.get('createTime'),
...
targets=[
 ('@depois.dos.trinta', [('video','7596460518949260562')]),
 ('@camilapereirams', [('video','7634612331527392533')]),
 ('@sire.aventuras', []),
 ('@alanchapgt', []),
]
out={}
for handle,medias in targets:
    ...
    time.sleep(4)
json.dump(out,open('verify6_results.json','w'),ensure_ascii=False,indent=1)
print('saved')
